...
export const calculateUserRanking = functions.firestore
...
export const updateGlobalRankings = functions.pubsub
...
async function calculateUserEngagement(userId: string) {
...
function getRankFromScore(score: number): string {
    ...
    if (score >= 0.85) return 'ROCKSTAR';
    if (score >= 0.70) return 'FAMOUS';
    if (score >= 0.55) return 'INFLUENCER';
    if (score >= 0.40) return 'POPULAR';
    if (score >= 0.25) return 'PARTY_POPPER';
    if (score >= 0.10) return 'BEGINNER';
    return 'NOVICE';
} 